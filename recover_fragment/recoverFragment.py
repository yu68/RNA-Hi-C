...
def main():
    ...
    primer2=primers[1]
    #---------------------------------------------------
    name=os.path.basename(args.input1).split('.')[0][:-2]
    ...
    for rec1, rec2 in itertools.izip(fastq_iter1,fastq_iter2):
        ...
        if (score1>=7*loc1[0]) or (score2>=7*loc2[0]):
            if (score1>=7*loc1[0]):
                ...
                fragment=SeqRecord(rec2[:(loc2[1]-loc2[0])].seq.reverse_complement(),id=rec2.id,name=rec2.name,description=rec2.description)
            ...
            elif (score>=8*min(loc)) and (loc[1]>loc[0]) and (loc[0]>=len(seq1)-6):
        ...
        total+=1
        if total%10000==0:
            print >> sys.stderr, " %d read pairs processed and there are %d type1, %d type2 and %d type3. \r "%(total, type1, type2, type3)
    output1.close()
    output2.close()
    output3.close()
    output4.close()
